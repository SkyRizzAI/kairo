...
> Kontrak kejujuran: PLP codec ada di DUA sisi (C++ device + TS Forge) dan itu
...
> **Insight kunci (dari diskusi):** protokol remote SELALU sama (PLP). Yang dibuat
...
1. **PLP codec (C++)** — frame, channel mux, RLE, crc8, handshake. Mirror byte-exact
   dari codec TS (`packages/forge/src/lib/plp/`, sudah ada) + **test vector bersama**.
...
4. **WASM firmware target** (`firmware/platforms/wasm/` + target) — firmware ASLI
   (core+apps+screens) jalan di browser via Emscripten, bicara PLP lewat
   `WasmCableTransport`. Inilah "simulator di WASM" sebagai **device PLP penuh**.
5. Semua teruji di **virtual-cable/loopback** tanpa hardware; BLE/USB menyusul.

---

## 2. PLP — wire format (kontrak, sudah ada di TS)
...
- `BleLinkTransport` — `send`=`IBleAdapter::notify(PLP_CHAR)`, `onWrite`→`RecvFn`. (Plan 34 BLE)
...
- **`RemoteService`** (`core/services/remote_service.{h,cpp}`) — `LinkService` (PLP
...
Firmware ASLI jalan di browser (Web Worker), bicara PLP via `WasmCableTransport`.
Inilah "simulator di WASM" = **device PLP penuh** (bukan protokol JSON-lines lama).

```
firmware/platforms/wasm/
├─ wasm_platform.{h,cpp}      # IPlatform: clock(emscripten), config(localStorage via JS), idle
├─ wasm_cable_transport.cpp   # postMessage ↔ PLP datagram
...
> Inilah jembatan ke "remote me-remote simulator": WASM = device yang expose PLP
...
| File | Aksi |
|---|---|
| `core/link/klp_codec.{h,cpp}` , `transport.h` | **baru** — PLP codec C++ + ILinkTransport |
...
| `core/link/klp_ble.h` | ✅ **baru** — UUID GATT PLP (kontrak dgn Forge `plp/uuids.ts`) |
| `platforms/esp32/.../esp32_ble.{h,cpp}` | ✅ **baru** — NimBLE radio + GATT PLP service |
...
1. **PLP codec C++ + test vector** — mirror `codec.ts`; `klp_test.cpp` sepadan
...
5. **BleLinkTransport** — bungkus IBleAdapter (34). Flash + tes PLP lewat BLE nyata.
...
- [ ] WASM: firmware asli jalan di Worker, expose PLP via virtual-cable (frame keluar, input masuk)
- [ ] BLE: setelah pair+handshake (34), PLP mengalir; LE Secure + token aktif
...
- **36** mengonsumsi: PLP codec TS (sudah ada di Forge), `nema.wasm`, dan
