...
> `build` → **satu file `.kapp`**, lalu **di-load di device** (Palanu). App jalan di
...
- Status: 🚧 **IN PROGRESS** (2026-06-08). **Fase 0 ✅** — `packages/nema-app-sdk`
  (`palanu`): jsx-runtime (auto), intrinsics (View/Text/Pressable/ScrollView/Slider/
  Row/Col), hooks (useState/useRef/useEffect), `renderToTree`, system-API types,
  `palanu-build` (Bun.build → single `.kapp`, `palanu` runtime external). Counter
  ...
  **Fase 2 ✅** — module loader (resolves `palanu` → embedded runtime header),
  ...
  **Fase 4 ✅** — `palanu.*` system API installed (capability-gated): `log`,
  `device{name,caps,has}`, `storage` (per-app config ns), `http.get` (blocking on
  the app thread, off UI). Wired into JsApp.
  **Fase 5 (Embedded) ✅** — `palanu-build` `.kapp` + `gen-embedded-apps` → C header;
...
SUDAH ada di Palanu: **rebuild pohon tiap render** (lihat ComponentApp). Maka:
- TSX dikompilasi (`jsx: "automatic"`, `jsxImportSource: "palanu"`) → `h()` calls.
...
TSX  ──build(esbuild)──► app.kapp (JS bundle + manifest)
   ...
   palanu.http/wifi/ble/... ◄── JS host functions ──► Runtime services (TaskRunner)
```
...
1. **DX**: `npm create palanu-app` → tulis `App.tsx` → `palanu-build` → `app.kapp`
...
3. **System API ter-expose & capability-gated**: `palanu.log`, `palanu.http`,
   `palanu.wifi`, `palanu.ble`, `palanu.storage`, `palanu.timer`, `palanu.device`.
...
### 2.1 `packages/nema-app-sdk` (npm, DX layer)
```
packages/nema-app-sdk/
  package.json            name: "palanu"  (import { View, Text, useState } from "palanu")
  ...
  src/system.d.ts         deklarasi global `palanu` (http/wifi/ble/storage/timer/log)
  src/manifest.ts         tipe KappManifest
  bin/palanu-build.ts      esbuild wrapper → bundle + manifest → app.kapp
...
- TSX → `h(type, props, ...children)`; `type` = string intrinsic ("View") atau
  ...
  tree-shaken; tanpa Node API; global `palanu` dianggap ambient (disediakan host).
...
```
firmware/core/include/palanu/js/
  ...
  js_api.h           // daftar host-function (palanu.*) + capability gating
...
### 2.3 System API (`palanu.*`) — host functions
- Disuntik saat context dibuat, **hanya** yang lolos capability:
  `palanu.log(level,tag,msg)`, `palanu.device.{name,caps}`,
  `palanu.http.get/post(url,opts) → Promise`,
  `palanu.wifi.{scan,connect,status}`, `palanu.ble.{advertise,onData,…}`,
  `palanu.storage.{get,set,remove}` (per-app namespace di config store),
  `palanu.timer.{setTimeout,setInterval,clear}`.
...
| Fase | Isi | Tes |
|---|---|---|
| **0. SDK + build** | `packages/nema-app-sdk`: `h()`, intrinsics, hooks, tipe, `palanu-build` (esbuild), template counter | bundle TSX→satu file; snapshot output; `bun run build` hijau |
...
```tsx
// App.tsx  (custom app)
import { View, Text, Pressable, useState } from "palanu";
  ...
  return (
    <View style={{ flexDirection: "column", padding: 4, gap: 6, alignItems: "center" }}>
      ...
      <Pressable onPress={async () => {
        const r = await palanu.http.get("https://api.example.com/x");
        palanu.log("info", "App", r.status);
...
- [ ] `palanu-build` mengubah TSX → satu `.kapp`; tipe TS jalan di editor.
