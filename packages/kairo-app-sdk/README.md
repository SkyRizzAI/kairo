...
```bash
bun bin/kairo-build.ts path/to/app        # → <id>.kapp (single file)
```
...
## System API (`kairo.*`, capability-gated)
```ts
kairo.log(level, tag, msg);
kairo.device.name; kairo.device.caps; kairo.device.has("wifi");
kairo.storage.get(key); kairo.storage.set(key, val); kairo.storage.remove(key);  // per-app
await kairo.http.get(url);   // { status, body } — present only if the board networks
```
A method is only present if the board supports it — guard with `kairo.device.has(...)`.
...
- `templates/sysinfo` — `kairo.device`/`storage`/`log` + a scrollable caps list.
