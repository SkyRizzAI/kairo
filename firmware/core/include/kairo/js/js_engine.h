...
// the embedded `kairo` runtime (jsx/components/hooks/renderToTree) via a module
...
namespace kairo::ui { struct UiNode; class NodeArena; struct ScrollState; }
...
public:
    ...
    // Load a custom-app bundle (ES module). Resolves `kairo` imports to the
    ...
    // Wire host services + install the capability-gated `kairo` global (system
    // API: log/device/storage/http…). Call before loadApp(). (Plan 37 Fase 4.)
    void setHost(kairo::Runtime* rt, std::string appId);
    kairo::Runtime*    host()  const { return host_; }
    const std::string& appId() const { return appId_; }

    // Embedded-runtime module caching (used by the module loader).
    JSModuleDef* kairoModuleDef() const { return kairoDef_; }
    void         setKairoModuleDef(JSModuleDef* d) { kairoDef_ = d; }
...
private:
    ...
    void     pumpJobs();
    void     freeHandlers();
    void     installApi();          // build the `kairo` global (js_api.cpp)
    ui::UiNode* reify(JSValueConst node, ui::NodeArena& arena);

    kairo::Runtime* host_ = nullptr;
    std::string     appId_;
    ...
    JSValue      renderFn_     = JS_UNDEFINED;   // kairo.renderToTree
    JSValue      scheduleFn_   = JS_UNDEFINED;   // C fn passed to renderToTree
    JSModuleDef* kairoDef_     = nullptr;        // cached embedded-runtime module
