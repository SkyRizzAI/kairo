...
// absent. Palanu owns the single HWCDC instance instead; boot banners and the PLP
// transport below must both go through this accessor.
HWCDC& usbSerialJtag();

// Esp32UsbCdc — IUsbCdc over the ESP32-S3 USB-Serial-JTAG (usbSerialJtag()). A
// raw byte pipe for PLP (Plan 34/35/37). A reader task polls the port and fans
// bytes to onData(). The PLP FrameParser tolerates interleaved console text via
